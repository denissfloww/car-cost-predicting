...
export const getBrands = async (req: Request, res: Response) => {
    try {
        const brands = await getRepository(car).createQueryBuilder('c').select('c.brand').distinctOn(['c.brand']).getMany();
...
export const getModels = async (req: Request, res: Response) => {
    try {
        const models = await getRepository(car).createQueryBuilder('c').select('c.model').distinctOn(['c.model']).getMany();
...
export const getFuelTypes = async (req: Request, res: Response) => {
    try {
        const fuelTypes = await getRepository(car).createQueryBuilder('c').select('c.fueltype').distinctOn(['c.fueltype']).getMany();
